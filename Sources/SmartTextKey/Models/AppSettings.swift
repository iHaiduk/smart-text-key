    ...
    private init() {
        ...
        if let data = defaults.data(forKey: "apiConfigs"),
           let decoded = try? JSONDecoder().decode([APIConfig].self, from: data) {
            ...
            let localOllama = APIConfig(name: "Local Ollama", apiBaseURL: "http://localhost:11434/v1", apiKey: "", modelName: "llama3")
            let openaiCloud = APIConfig(name: "OpenAI Cloud", apiBaseURL: "https://api.openai.com/v1", apiKey: "", modelName: "gpt-4o")
